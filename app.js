...
function AfficherListProduits() {
  ...
  produits.forEach((produit) => {
    ...
    card.className =
      "produit-card col-md-3 border border-2 rounded p-3 m-2 text-center";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    return [
      {
        id: "1",
        nom: "Banane",
        description: "Banane locale, douce et sucrée.",
        image: "images/téléchargement.jpeg",
        prix: 3500,
      },
      {
        id: "2",
        nom: "Mangue",
        description: "Mangue juteuse de saison.",
        image: "images/mange.jpeg",
        prix: 4500,
      },
      {
  ...
  function createCard(p) {
    ...
    card.innerHTML = `
      ...
      <div class="actions">
        ...
        <a class="btn btn-outline-secondary ms-1" href="card.html?id=${encodeURIComponent(
          card.dataset.id
  ...
  function extractProductFromCard(cardEl) {
    return {
      ...
      prix:
        parseFloat(
          (cardEl.querySelector(".prix").textContent.match(/[\d.,]+/) || [
            "0",
          ])[0].replace(",", ".")
        ) || 0,
    };
  ...
  function addToCart(product) {
    ...
    // recompute totalQty
    let tot = 0;
    for (const k in cart.items) tot += cart.items[k].qty;
    cart.totalQty = tot;
    saveCart(cart);
    renderBadge();
  }
  ...
  if (btnNewProduct)
    btnNewProduct.addEventListener("click", () => window.addProductPrompt());
